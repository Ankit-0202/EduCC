...
std::unique_ptr<IntermediateRepresentation>
IRGenerator::generate(std::unique_ptr<Parser::ASTNode> &ast) {
...
std::string
IRGenerator::visit(Parser::ASTNode *node,
                   std::unique_ptr<IntermediateRepresentation> &ir) {
  if (!node)
    return "";
  ...
  case Parser::ASTNodeType::ASSIGNMENT_EXPRESSION:
    return visitAssignmentExpression(
        static_cast<Parser::AssignmentExpression *>(node), ir);
  case Parser::ASTNodeType::BINARY_EXPRESSION:
    return visitBinaryExpression(static_cast<Parser::BinaryExpression *>(node),
                                 ir);
  case Parser::ASTNodeType::UNARY_EXPRESSION:
    return visitUnaryExpression(static_cast<Parser::UnaryExpression *>(node),
                                ir);
...
// Visit Program node
void IRGenerator::visitProgram(
    Parser::Program *program, std::unique_ptr<IntermediateRepresentation> &ir) {
...
void IRGenerator::visitFunctionDefinition(
    Parser::FunctionDefinition *funcDef,
    std::unique_ptr<IntermediateRepresentation> &ir) {
  // Add function label
...
void IRGenerator::visitVariableDeclaration(
    Parser::VariableDeclaration *varDecl,
    std::unique_ptr<IntermediateRepresentation> &ir) {
  if (varDecl->initializer) {
    ...
    ir->addInstruction(IRInstruction(IRInstructionType::STORE_VAR, initVal, "",
                                     varDecl->name));
  } else {
    // Initialize variable to default value (e.g., 0)
    std::string temp = ir->newTemp();
    ir->addInstruction(
        IRInstruction(IRInstructionType::LOAD_CONST, "0", "", temp));
    ir->addInstruction(
        IRInstruction(IRInstructionType::STORE_VAR, temp, "", varDecl->name));
  }
}

// Visit Block node
void IRGenerator::visitBlock(Parser::Block *block,
                             std::unique_ptr<IntermediateRepresentation> &ir) {
...
void IRGenerator::visitIfStatement(
  ...
  ir->addInstruction(
      IRInstruction(IRInstructionType::CJMP, cond, "", labelTrue));
...
void IRGenerator::visitForStatement(
    Parser::ForStatement *forStmt,
    std::unique_ptr<IntermediateRepresentation> &ir) {
  // Generate labels
  ...
  ir->addInstruction(
      IRInstruction(IRInstructionType::JMP, "", "", labelCondition));
  ...
  if (forStmt->condition) {
    ...
    ir->addInstruction(
        IRInstruction(IRInstructionType::CJMP, cond, "", "loop_body"));
  } else {
    // If no condition, create an infinite loop
    ir->addInstruction(
        IRInstruction(IRInstructionType::JMP, "", "", "loop_body"));
  }
...
void IRGenerator::visitWhileStatement(
    Parser::WhileStatement *whileStmt,
    std::unique_ptr<IntermediateRepresentation> &ir) {
  // Generate labels
  std::string labelCondition = ir->newLabel();
  ...
  ir->addInstruction(
      IRInstruction(IRInstructionType::CJMP, cond, "", labelBody));
...
void IRGenerator::visitReturnStatement(
    Parser::ReturnStatement *retStmt,
    std::unique_ptr<IntermediateRepresentation> &ir) {
  if (retStmt->expression) {
    ...
    ir->addInstruction(
        IRInstruction(IRInstructionType::LOAD_CONST, retVal, "", "rax"));
  } else {
    // No return expression; default to 0
    ir->addInstruction(
        IRInstruction(IRInstructionType::LOAD_CONST, "0", "", "rax"));
  }
...
void IRGenerator::visitExpressionStatement(
    Parser::ExpressionStatement *exprStmt,
    std::unique_ptr<IntermediateRepresentation> &ir) {
...
std::string IRGenerator::visitAssignmentExpression(
    Parser::AssignmentExpression *assignExpr,
    std::unique_ptr<IntermediateRepresentation> &ir) {
  // Ensure left side is an identifier
  auto identifier = dynamic_cast<Parser::Identifier *>(assignExpr->left.get());
  if (!identifier) {
    std::cerr
        << "IRGenerator: Left-hand side of assignment is not an identifier.\n";
    return "";
  }
  ...
  // Store the computed value into the variable
  ir->addInstruction(
      IRInstruction(IRInstructionType::STORE_VAR, rhs, "", varName));

  return varName;
}

// Visit Binary Expression node
std::string IRGenerator::visitBinaryExpression(
    Parser::BinaryExpression *binExpr,
    std::unique_ptr<IntermediateRepresentation> &ir) {
...
std::string IRGenerator::visitUnaryExpression(
    Parser::UnaryExpression *unaryExpr,
    std::unique_ptr<IntermediateRepresentation> &ir) {
  ...
  if (unaryExpr->op == "-") {
    ...
    ir->addInstruction(
        IRInstruction(IRInstructionType::SUB, "0", operand, temp));
  } else if (unaryExpr->op == "!") {
    // Implement logical NOT (assuming !x is equivalent to x == 0)
    std::string zeroTemp = ir->newTemp();
    ir->addInstruction(
        IRInstruction(IRInstructionType::LOAD_CONST, "0", "", zeroTemp));
    ir->addInstruction(
        IRInstruction(IRInstructionType::SUB, operand, zeroTemp, temp));
...
// Visit Literal node
std::string
IRGenerator::visitLiteral(Parser::Literal *literal,
                          std::unique_ptr<IntermediateRepresentation> &ir) {
...
std::string
IRGenerator::visitIdentifier(Parser::Identifier *identifier,
                             std::unique_ptr<IntermediateRepresentation> &ir) {
  // Generate a temporary to hold the loaded value
  std::string temp = ir->newTemp();
  ir->addInstruction(
      IRInstruction(IRInstructionType::LOAD_VAR, identifier->name, "", temp));
  return temp;
}
