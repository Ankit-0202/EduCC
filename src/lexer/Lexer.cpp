...
const std::unordered_map<std::string, TokenType> keywords = {
    ...
    {"double", TokenType::DOUBLE}, {"void", TokenType::VOID},
...
bool Lexer::isAtEnd() const { return currentIndex >= sourceCode.length(); }
    ...
    if (std::isspace(c)) {
      ...
      if (currentIndex + 1 < sourceCode.length()) {
        ...
        if (next == '/') {
          ...
          getNextChar(); // Consume '*'
          while (!isAtEnd()) {
            if (peek() == '*' && currentIndex + 1 < sourceCode.length() &&
...
Token Lexer::operatorOrDelimiter() {
  ...
  case '+':
    return Token(TokenType::PLUS, lexeme, startLine, startColumn);
  case '-':
    return Token(TokenType::MINUS, lexeme, startLine, startColumn);
  case '*':
    return Token(TokenType::MULTIPLY, lexeme, startLine, startColumn);
  case '/':
    return Token(TokenType::DIVIDE, lexeme, startLine, startColumn);
  case ';':
    return Token(TokenType::SEMICOLON, lexeme, startLine, startColumn);
  case ',':
    return Token(TokenType::COMMA, lexeme, startLine, startColumn);
  case '(':
    return Token(TokenType::LPAREN, lexeme, startLine, startColumn);
  case ')':
    return Token(TokenType::RPAREN, lexeme, startLine, startColumn);
  case '{':
