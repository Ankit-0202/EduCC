    ...
    if (check(Lexer::TokenType::IDENTIFIER)) {
      ...
      if (match(Lexer::TokenType::LPAREN)) {
        ...
        if (!match(Lexer::TokenType::RPAREN)) {
          error("Expected ')' after function parameters.", peek().line,
                peek().column);
        }
        ...
        if (match(Lexer::TokenType::LBRACE)) {
          ...
          return std::make_unique<FunctionDefinition>(type, name, parameters,
                                                      std::move(body));
        } else {
          // Function declaration without body (prototype) is not handled in
          // this simplified parser
          error("Function declaration without body is not supported.",
        ...
        if (!match(Lexer::TokenType::SEMICOLON)) {
          error("Expected ';' after variable declaration.", peek().line,
                peek().column);
        }
      ...
      error("Expected identifier after type specifier.", peek().line,
            peek().column);
    }
  }
...
std::unique_ptr<Expression> Parser::parseAssignmentExpression() {
  ...
  if (match(Lexer::TokenType::ASSIGN)) {
    ...
    return std::make_unique<AssignmentExpression>(std::move(lhs),
                                                  std::move(rhs));
  }
...
// Parse primary expressions (identifiers, literals, and parenthesized
// expressions)
...
void Parser::error(const std::string &message, int line, int column) {
  std::cerr << "[Parser Error] " << message << " at line " << line
            << ", column " << column << "\n";
