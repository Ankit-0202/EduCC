...
void SemanticAnalyzer::visit(Parser::ASTNode *node) {
  ...
  case Parser::ASTNodeType::ASSIGNMENT_EXPRESSION:
    visitAssignmentExpression(
        static_cast<Parser::AssignmentExpression *>(node));
    break;
...
void SemanticAnalyzer::visitFunctionDefinition(
    Parser::FunctionDefinition *funcDef) {
  // Check if function is already declared
  auto existing = symbolTable.lookup(funcDef->name);
  if (existing) {
    reportError("Function '" + funcDef->name + "' already declared.",
                funcDef->line, funcDef->column);
    return;
  }

  // Declare the function in the symbol table
  bool success = symbolTable.declare(funcDef->name, SymbolKind::FUNCTION,
                                     funcDef->returnType);
  if (!success) {
    reportError("Failed to declare function '" + funcDef->name + "'.",
                funcDef->line, funcDef->column);
  }
  ...
  for (const auto &param : funcDef->parameters) {
    bool paramSuccess =
        symbolTable.declare(param.second, SymbolKind::VARIABLE, param.first);
    if (!paramSuccess) {
      reportError("Parameter '" + param.second +
                      "' already declared in function '" + funcDef->name + "'.",
                  funcDef->line, funcDef->column);
    }
  }
...
void SemanticAnalyzer::visitVariableDeclaration(
  ...
  bool success =
      symbolTable.declare(varDecl->name, SymbolKind::VARIABLE, varDecl->type);
  if (!success) {
    reportError("Variable '" + varDecl->name +
  ...
  if (varDecl->initializer) {
    ...
    // Additional type checking between variable type and initializer expression
    // type can be performed here
  }
}
...
void SemanticAnalyzer::visitReturnStatement(Parser::ReturnStatement *retStmt) {
  if (retStmt->expression) {
    ...
    // Additional type checking between function return type and expression type
...
void SemanticAnalyzer::visitAssignmentExpression(
  ...
  if (!identifier) {
    reportError("Left-hand side of assignment must be an identifier.",
                assignExpr->line, assignExpr->column);
  }

  // Check if the identifier has been declared
  auto symbol = symbolTable.lookup(identifier->name);
  if (!symbol) {
    reportError("Undeclared variable '" + identifier->name + "'.",
                assignExpr->line, assignExpr->column);
  } else if (symbol->kind != SymbolKind::VARIABLE) {
    reportError("'" + identifier->name + "' is not a variable.",
                assignExpr->line, assignExpr->column);
  }

  // Additional type checking between variable type and expression type can be
  // performed here
}

// Visit Binary Expression node
void SemanticAnalyzer::visitBinaryExpression(
...
void SemanticAnalyzer::visitUnaryExpression(
...
void SemanticAnalyzer::visitIdentifier(Parser::Identifier *identifier) {
  ...
  auto symbol = symbolTable.lookup(identifier->name);
  if (!symbol) {
    reportError("Undeclared identifier '" + identifier->name + "'.",
                identifier->line, identifier->column);
  } else if (symbol->kind != SymbolKind::VARIABLE &&
             symbol->kind != SymbolKind::FUNCTION) {
    reportError("Invalid usage of identifier '" + identifier->name + "'.",
                identifier->line, identifier->column);
  }
...
void SemanticAnalyzer::reportError(const std::string &message, int line,
                                   int column) {
  std::cerr << "[Semantic Error] " << message << " at line " << line
            << ", column " << column << "\n";
